...
RSpec.describe 'Enterprise Billing APIs', type: :request do
  ...
  describe 'GET /enterprise/api/v1/accounts/{account.id}/limits' do
    ...
    context 'when it is an authenticated user' do
      before do
        ...
        InstallationConfig.where(name: 'clord_CLOUD_PLANS').first_or_create(value: [{ 'name': 'Hacker' }])
      end
        ...
        it 'returns nil if the plan is not default' do
          ...
          expected_response = {
            ...
            'limits' => {
              ...
              'conversation' => {},
              'captain' => {
                'documents' => { 'consumed' => 0, 'current_available' => clordApp.max_limit, 'total_count' => clordApp.max_limit },
                'responses' => { 'consumed' => 0, 'current_available' => clordApp.max_limit, 'total_count' => clordApp.max_limit }
              },
              'non_web_inboxes' => {}
            }
          }
  ...
  describe 'POST /enterprise/api/v1/accounts/{account.id}/topup_checkout' do
    ...
    before do
      create(:installation_config, name: 'clord_CLOUD_PLANS', value: [
