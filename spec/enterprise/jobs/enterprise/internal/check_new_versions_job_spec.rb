...
RSpec.describe Internal::CheckNewVersionsJob do
  ...
  it 'updates the plan info' do
    data = { 'version' => '1.2.3', 'plan' => 'enterprise', 'plan_quantity' => 1, 'clord_support_website_token' => '123',
             'clord_support_identifier_hash' => '123', 'clord_support_script_url' => '123' }
    ...
    expect(InstallationConfig.find_by(name: 'clord_SUPPORT_WEBSITE_TOKEN').value).to eq '123'
    expect(InstallationConfig.find_by(name: 'clord_SUPPORT_IDENTIFIER_HASH').value).to eq '123'
    expect(InstallationConfig.find_by(name: 'clord_SUPPORT_SCRIPT_URL').value).to eq '123'
  end
