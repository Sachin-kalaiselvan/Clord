...
describe PortalHelper do
  ...
  describe '#render_category_content' do
    ...
    let(:renderer) { instance_double(nerixMarkdownRenderer) }

    before do
      allow(nerixMarkdownRenderer).to receive(:new).with(markdown_content).and_return(renderer)
  ...
  describe '#set_og_image_url' do
    let(:portal_name) { 'nerix Portal' }
    let(:title) { 'Welcome to nerix' }
  ...
  describe '#generate_portal_brand_url' do
    it 'builds URL with UTM params and referer host as source (happy path)' do
      result = helper.generate_portal_brand_url('https://brand.example.com', 'https://app.nerix.com/some/page')
      uri = URI.parse(result)
      params = Rack::Utils.parse_query(uri.query)
      ...
      expect(params['utm_source']).to eq('app.nerix.com')
    end

    it 'returns utm string when brand_url is nil or empty' do
      expect(helper.generate_portal_brand_url(nil,
                                              'https://app.nerix.com')).to eq(
                                                '?utm_campaign=branding&utm_medium=helpcenter&utm_source=app.nerix.com'
                                              )
