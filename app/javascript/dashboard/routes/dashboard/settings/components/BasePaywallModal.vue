...
<template>
  ...
  >
    ...
    <p class="text-sm font-normal text-n-slate-11">
      ...
      <span v-if="isOnnerixCloud && !isAdmin">
        {{ $t('GENERAL_SETTINGS.LIMIT_MESSAGES.NON_ADMIN') }}
      </span>
      <span v-else-if="!isOnnerixCloud && !isSuperAdmin">
        {{ $t(`${featurePrefix}.ENTERPRISE_PAYWALL.ASK_ADMIN`) }}
      </span>
    </p>
    <template v-if="isOnnerixCloud && isAdmin">
    ...
    <template v-else-if="!isOnnerixCloud && isSuperAdmin">
