...
const { isOnnerixCloud } = useAccount();
...
const fetchSSLStatus = () => {
  if (!isOnnerixCloud.value) return;
...
const handleUpdatePortalConfiguration = portalObj => {
  ...
  // If custom domain is added or updated, fetch SSL status after a delay of 5 seconds (only on nerix cloud)
  if (portalObj?.custom_domain && isOnnerixCloud.value) {
    setTimeout(() => {
      fetchSSLStatus();
