...
export const setBubbleText = bubbleText => {
  if (isExpandedView(window.$nerix.type)) {
...
export const createBubbleIcon = ({ className, path, target }) => {
  let bubbleClassName = `${className} woot-elements--${window.$nerix.position}`;
...
const handleBubbleToggle = newIsOpen => {
  ...
  if (newIsOpen) {
    dispatchWindowEvent({ eventName: nerix_OPENED });
  } else {
    dispatchWindowEvent({ eventName: nerix_CLOSED });
    chatBubble.focus();
  }
};
...
export const onBubbleClick = (props = {}) => {
  ...
  const { isOpen } = window.$nerix;
  if (isOpen === toggleValue) return;

  const newIsOpen = toggleValue === undefined ? !isOpen : toggleValue;
  window.$nerix.isOpen = newIsOpen;
