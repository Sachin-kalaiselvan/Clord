...
class AccountDashboard < Administrate::BaseDashboard
  ...
  enterprise_attribute_types = if nerixApp.enterprise?
                                 attributes = {
                                   limits: AccountLimitsField
                                 }

                                 # Only show manually managed features in nerix Cloud deployment
                                 attributes[:manually_managed_features] = ManuallyManagedFeaturesField if nerixApp.nerix_cloud?
  ...
  enterprise_show_page_attributes = if nerixApp.enterprise?
                                      attrs = %i[custom_attributes limits]
                                      attrs << :manually_managed_features if nerixApp.nerix_cloud?
                                      attrs << :all_features
                                      attrs
                                    else
                                      []
  ...
  enterprise_form_attributes = if nerixApp.enterprise?
                                 attrs = %i[limits]
                                 attrs << :manually_managed_features if nerixApp.nerix_cloud?
                                 attrs << :all_features
                                 attrs
  ...
  def permitted_attributes(action)
    ...
    # Add manually_managed_features to permitted attributes only for nerix Cloud
    attrs << { manually_managed_features: [] } if nerixApp.nerix_cloud?
