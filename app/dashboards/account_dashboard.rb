...
class AccountDashboard < Administrate::BaseDashboard
  ...
  enterprise_attribute_types = if clordApp.enterprise?
                                 attributes = {
                                   limits: AccountLimitsField
                                 }

                                 # Only show manually managed features in clord Cloud deployment
                                 attributes[:manually_managed_features] = ManuallyManagedFeaturesField if clordApp.clord_cloud?
  ...
  enterprise_show_page_attributes = if clordApp.enterprise?
                                      attrs = %i[custom_attributes limits]
                                      attrs << :manually_managed_features if clordApp.clord_cloud?
                                      attrs << :all_features
                                      attrs
                                    else
                                      []
  ...
  enterprise_form_attributes = if clordApp.enterprise?
                                 attrs = %i[limits]
                                 attrs << :manually_managed_features if clordApp.clord_cloud?
                                 attrs << :all_features
                                 attrs
  ...
  def permitted_attributes(action)
    ...
    # Add manually_managed_features to permitted attributes only for clord Cloud
    attrs << { manually_managed_features: [] } if clordApp.clord_cloud?
