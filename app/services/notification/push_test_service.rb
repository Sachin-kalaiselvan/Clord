class Notification::PushTestService
  ...
  def self.default_title
    format(DEFAULT_TITLE, installation_name: GlobalConfigService.load('INSTALLATION_NAME', 'nerix'))
  end
  ...
  def test_fcm_via_hub(subscription)
    response = nerixHub.send_push_with_response(fcm_options(subscription))
